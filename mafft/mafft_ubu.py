...
# The path of mafft file.
mafft_exe = "/usr/bin/mafft"

#This line prepares the command to be used in mafft application. 
mafft_cline = MafftCommandline(mafft_exe, input=os.path.join(base_dir, "input.fasta"))

#This line is to save the starting time of mafft 
timeStarted = time.time() 

#This line runs the command to perform the alignment process
child = subprocess.Popen(str(mafft_cline) + " > " + os.path.join(base_dir, "output.fasta"), shell=True, universal_newlines=True)

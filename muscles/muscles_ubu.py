...
# The path of muscle file.
muscle_exe = "/usr/bin/muscle3.8.31_i86linux64"

#This line prepares the command to be used in muscle application. 
muscle_cline = MuscleCommandline(muscle_exe, input=os.path.join(base_dir, "input.fasta"), out=os.path.join(base_dir, "output.fasta"))

#This line is to save the starting time of mafft 
timeStarted = time.time() 

#This line runs the command to perform the alignment process
child = subprocess.Popen(str(muscle_cline), shell=True, universal_newlines=True)

...
def usage():
    ...
    To combine multiple alignments:
        % python MergeAlign.py -a path/to/folder
...
def convertListOfCoordinatesToSequences(final_coordinates, original_sequences):
    ...
    for i, sequence in enumerate(list(zip(*final_coordinates))):
        seq = seq_names[i]
...
def scoreNodes(nodes, num_paths=100):
    ...
    dimensions = len(list(nodes.keys())[0])
    first_node = tuple([0]*dimensions)
    
    for coord, current_node in sorted(nodes.items())[1:]:
...
def outputAlignmentAsFASTA(filename, final_alignment, threshold=None, scores=None):
    ...
    for name, sequence in final_alignment.items():
        output.write(">%s\n" % name)
        if threshold:
            sequence = ''.join([aa for (aa, score) in list(zip(sequence, scores)) if score>threshold])
        output.write("%s\n" % sequence)
...
def outputAlignmentVertical(filename, final_alignment, original_sequences):
    ...
    for i, sequence in enumerate(list(zip(*[residue for (residue, score) in final_alignment]))):
...
if __name__ == '__main__':
    ...
    #This line is to save the starting time of mcoffee 
    ...
    try:                                
        opts, args = getopt.getopt(sys.argv[1:], "ha:f:s:t:", ["help", "alignments=", "fasta=", "score=", "threshold="])
    ...
    print("Max CPU Usage : {}".format(cpu_percent))
    print("Max Memory Usage : {}".format(mem_percent))